...
void VS1053_t::begin() {
  ...
  if (testComm(false)) {
    ...
    ESP_LOGD(TAG, "spi_bus_add_device=%d", ret);
    assert(ret == ESP_OK);
    write_register(SCI_MODE, _BVV(SM_SDINEW) | _BVV(SM_LINE1));
...
bool VS1053_t::current_data_request() {
  return (gpio_get_level((gpio_num_t)dreq_pin) == 1);
}

void VS1053_t::control_mode_on() {
  gpio_set_level((gpio_num_t)dcs_pin, 1);  // Bring slave in control mode
  gpio_set_level((gpio_num_t)cs_pin, 0);
}

void VS1053_t::control_mode_off() {
  gpio_set_level((gpio_num_t)cs_pin, 1);  // End control mode
}

void VS1053_t::data_mode_on() {
  gpio_set_level((gpio_num_t)cs_pin, 1);  // Bring slave in data mode
  gpio_set_level((gpio_num_t)dcs_pin, 0);
}

void VS1053_t::data_mode_off() {
  gpio_set_level((gpio_num_t)dcs_pin, 1);  // End data mode
}
...
void begin() {
	...
	digitalWrite(dcs_pin, 1); // Start HIGH for SCI en SDI
	digitalWrite(cs_pin, 1);
	delay(100);
	LOG("\n");
	LOG("Reset VS1053...\n");
	digitalWrite(dcs_pin, 0); // Low & Low will bring reset pin low
	digitalWrite(cs_pin, 0);
	delay(500);
	LOG("End reset VS1053...\n");
	digitalWrite(dcs_pin, 1); // Back to normal again
	digitalWrite(cs_pin, 1);
	delay(500);
...
void VS1053_t::stopSong() {
  ...
  delay(10);
  write_register(SCI_MODE, _BVV(SM_SDINEW) | _BVV(SM_CANCEL));
  for (i = 0; i < 200; i++) {
    sdi_send_fillers(32);
    modereg = read_register(SCI_MODE);  // Read status
    if ((modereg & _BVV(SM_CANCEL)) == 0) {
      sdi_send_fillers(2052);
...
void VS1053_t::softReset() {
  ...
  write_register(SCI_MODE, _BVV(SM_SDINEW) | _BVV(SM_RESET));
  delay(10);
  await_data_request();
}
